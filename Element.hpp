...
private:
    ...
    double        ForcaSuperficieFxno1; // valores adicionados
    double        ForcaSuperficieFyno1;
    double        ForcaSuperficieFxno2;
    double        ForcaSuperficieFyno2;
    double        ForcaSuperficieFxno3;
    double        ForcaSuperficieFyno3;
...
public:
    ...
    Element(int index, Connectivity& connect, std::vector<Nodes *> &nodes){
        index_ = index; //Element index
        connect_ = connect; //Velocity mesh connectivity 
        nodes_ = nodes; //Velocity nodes
    ...
    /// forca de superficie no1
    double getForcaSuperficieFxno1(){return ForcaSuperficieFxno1;}; // valores adicionados
    double getForcaSuperficieFyno1(){return ForcaSuperficieFyno1;};

    /// forca de superficie no2
    double getForcaSuperficieFxno2(){return ForcaSuperficieFxno2;};
    double getForcaSuperficieFyno2(){return ForcaSuperficieFyno2;};

    /// forca de superficie no3
    double getForcaSuperficieFxno3(){return ForcaSuperficieFxno3;};
    double getForcaSuperficieFyno3(){return ForcaSuperficieFyno3;};
...
void Element<2>::computeDragAndLiftForces() {
    ...
    // calculando e armazenando coordenadas dos nodes do elemento 1D do contorno
    ...
    ublas::bounded_vector<double,2>            fsup_vector; // adicionados
    DimMatrix                                  cauchyStress;
    ...
    int index = 0;
    // laco nos pontos de quadratura
    for(typename BoundaryQuad::QuadratureListIt it = bQuad.begin(); 
        it != bQuad.end(); it++){
        
        double xsiB = gaussQuad.first(index); // xsiB calculado para linha
        double weightB = gaussQuad.second(index);
        
        // coordenadas adimensionais nos pontos de integracao em funcao do lado do elemento
        if(sideBoundary_ == 2){
            // nodes {0,3,1}
            xsi(0) = (-xsiB + 1.) / 2.; // parece que ele escreveu os pontos de quadratura 2D em funcao da coordenada 1D de gauss
            xsi(1) = 0.; // xsi para elemento triangular
        };
        if(sideBoundary_ == 1){
            // nodes {2,5,0}
            xsi(1) = (xsiB + 1.) / 2.;
        ...
        shapeQuad.evaluate(xsi,phi_); // verificar se esta avaliacao ja vale automaticamente para dphi-> du_dx? E se eu precisar trocar xsi?
        ...
        // Versor normal a superficie do fluido? (sinal oposto ao do solido)
        n_vector(0) =  Ty / jacb_;
        n_vector(1) = -Tx / jacb_; // ele esta integrando no sentido horario
        
        // tensor de desviador
        shearStress(0,0) = 2. * visc_ * du_dx; // verificar: nao depende da coordenada ksi da iteracao atual?
        shearStress(0,1) = visc_ * (du_dy + dv_dx);
        shearStress(1,0) = visc_ * (du_dy + dv_dx);
        shearStress(1,1) = 2. * visc_ * dv_dy;


        // daria para integrar as forcas de superficie
        load_pressure += -p_ * prod(ident,n_vector) * jacb_ * weightB; // integrando tensor de pressao diagonal
        load_friction += prod(shearStress,n_vector) * jacb_ * weightB; // integrando tensor de tensoes viscosas
    ...
    pressureDragForce = load_pressure(0); //pDForce = elements_[j] -> getPressureDragForce(); 
    ...
    // Calculo da forca de superficie por node -----------------------------------------------
     n_vector.clear();
     shearStress.clear();

     fsup_vector.clear(); // adicionados
     cauchyStress.clear();

        int elem_Fsup=index;
        int noLocalElem2D=index; // index is global; 
        ...
        if(sideBoundary_ == 2){
            ...
            if (no==0){
                ...
                // node 5
                xsi(0) = 0.0; // coordenadas do elemento triangular quadratico
                xsi(1) = 0.5;
            ...
            if (no==0){
                ...
                // node 4
                xsi(0) = 0.5; // xsi para elemento triangular
                xsi(1) = 0.5;
        ...
        // Versor normal a superficie do fluido? (sinal oposto ao do solido)
        n_vector(0) =  Ty / jacb_; // verificar sinal da integracao
        n_vector(1) = -Tx / jacb_; // ele esta integrando no sentido horario
        
        // tensor desviador
        shearStress(0,0) = 2. * visc_ * du_dx; // supondo  du_dx seja atualizado para xsi
        shearStress(0,1) = visc_ * (du_dy + dv_dx);
        shearStress(1,0) = visc_ * (du_dy + dv_dx);
        shearStress(1,1) = 2. * visc_ * dv_dy;

        // tensor de tensoes de Cauchy: sigma= tensorHidrostatico + tensorDesviador
        cauchyStress(0,0) = shearStress(0,0) -p_; // valores adicionados
        cauchyStress(0,1) = shearStress(0,1);
        cauchyStress(1,0) = shearStress(1,0);
        cauchyStress(1,1) = shearStress(1,1) -p_;

        // Vetor local de Forcas de superficie: tSUp=sigmaTransposto . n_vector_solido
        fsup_vector = prod(cauchyStress,n_vector); // verificar se precisa transpor
        
        if (no==0){
            ForcaSuperficieFxno1 = fsup_vector(0); // valores adicionados
            ForcaSuperficieFyno1 = fsup_vector(1);

        } else if (no==1){
            ForcaSuperficieFxno2 = fsup_vector(0);
            ForcaSuperficieFyno2 = fsup_vector(1);

        } else if (no==2){
            ForcaSuperficieFxno3 = fsup_vector(0);
            ForcaSuperficieFyno3 = fsup_vector(1);
        }
